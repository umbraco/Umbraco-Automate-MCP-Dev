...
DB_NAME="umbraco-mcp-automate-$DIR_SLUG"
...
if [ -f "$PROJECT_DIR/demo-site/appsettings.local.json" ]; then
  SA_PASSWORD=$(jq -r '.ConnectionStrings.umbracoDbDSN // ""' "$PROJECT_DIR/demo-site/appsettings.local.json" | sed -nE 's/.*[Pp]assword=([^;]*).*/\1/p')
fi
